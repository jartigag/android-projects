...
public class ContactFetcher {
    ...
    public ArrayList<Contact> fetchAll() {
        ...
        matchContactWebsite(contactsMap);
    ...
    public void matchContactWebsite(Map<String, Contact> contactsMap) {
        // Get Website
        final String[] websiteProjection = new String[]{
                Website.DATA,
                Website.CONTACT_ID,
        };

        Cursor website = new CursorLoader(context,
                Email.CONTENT_URI,
                websiteProjection,
                null,
                null,
        ...
        if (website.moveToFirst()) {
            final int contactWebsiteColumnIndex = website.getColumnIndex(Website.DATA);
            final int contactIdColumnsIndex = website.getColumnIndex(Website.CONTACT_ID);

            while (!website.isAfterLast()) {
                final String url = website.getString(contactWebsiteColumnIndex);
                final String contactId = website.getString(contactIdColumnsIndex);
                Contact contact = contactsMap.get(contactId);
                if (contact == null) {
                    continue;
                }
                contact.addWebsite(url);
                website.moveToNext();
            }
        }
